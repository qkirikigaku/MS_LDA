...
def main():
    ...
    if(args[1] == '1'):
        input_file = 'result/data' + str(data_type) + '_o' + threshold + '_' \
                   + cancer_type +'/result_k' + topic + '.txt'
    else:
        input_file = 'result/data' + str(data_type) + '_o' + threshold + '_' \
                   + cancer_type + '/figure/' + str(K) + '/k' + topic + '.txt'
    ...
    output_file = "result/data" + str(data_type) + '_o' + threshold + '_' \
    ...
    output_right_sig = open('result/data' + str(data_type) + '_o' + threshold \
            + '_' + cancer_type + '/figure/match_right_sig.txt' , 'w')
    ...
    output_sig = open('result/data' + str(data_type) + '_o' + threshold + '_' \
            + cancer_type + '/figure/match_sigs.txt', 'w')
    ...
    output_not_sig = open('result/data' + str(data_type) + '_o' + threshold \
                   + '_' + cancer_type + '/figure/not_match_predicted.txt', 'w')
    output_not_sig.write('0.50以上 :')
    for i in range(K):
        if(min_JS[i] >= 0.5):
            output_not_sig.write(str(int(i + 1)) + ', ')

    min_cos = open('result/data' + str(data_type) + '_o' + threshold + '_' \
            + cancer_type + '/figure/minimum_cos.txt', 'w')
    for i in range(K):
