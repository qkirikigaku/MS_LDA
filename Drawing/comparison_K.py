...
def main():
    i = 2
    while(i < K+1):
        string = 'result/data' + str(data_type) + '_o' + threshold + '_' \
               + cancer_type + '_' + str(iteration) + '/result_k'
        if(i <= 9):
            string += '0' + str(i) + '.txt'
            files[i-2] = string
        else:
    ...
    plt.tight_layout()
    name = 'result/data' + str(data_type) + '_o' + threshold + '_' \
         + cancer_type + '_' + str(iteration) +'/figure/ELBO.png'
    plt.savefig(name)
    plt.close(1)
    ...
    plt.tight_layout()
    name = 'result/data' + str(data_type) + '_o' + threshold + '_' \
         + cancer_type + '_' + str(iteration) + '/figure/ELBO_a.png'
    plt.savefig(name)
    plt.close(1)
    ...
    plt.tight_layout()
    name = 'result/data' + str(data_type) + '_o' + threshold + '_' \
         + cancer_type + '_' + str(iteration) + '/figure/Perplexity.png'
    plt.savefig(name)
    plt.close(1)
    ...
    plt.tight_layout()
    name = 'result/data' + str(data_type) + '_o' + threshold + '_' \
         + cancer_type + '_' + str(iteration) +'/figure/Perplexity_a.png'
    plt.savefig(name)
    plt.close(1)
