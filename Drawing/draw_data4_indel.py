...
def main():
    ...
    for i in range(K):
        ...
        ax1.set_xlim(0, 21)
        ax1.set_xticks(left)
        ax1.set_xlabel('indel', fontsize=7)
        ax1.set_xticklabels(labels)
        for tick in ax1.get_xticklabels():
            tick.set_rotation(90)
        ax1.tick_params(labelsize = 4)
        ax1.set_ylabel('p (indel = x)', fontsize=7)
